...
from swebench.harness.utils import load_swebench_dataset, load_swebench_dataset_local  # pyright: ignore[reportUnknownVariableType]
    ...
    async def rollout_async(
        ...
        try:
            ...
            prediction = controller.run_instance(
                task, max_turns=self.max_turns, run_method=cast(Literal["python", "cli", "openai_tools"], self.run_method)
            )
            del controller
...
async def run_instance_async(
    ...
    # ====== 调试打印：查看 Span 详细内容 ======
    logger.info(f"=== [DEBUG] Spans for {instance_id} (Total: {len(spans)}) ===")
    for i, span in enumerate(spans):
        try:
            # 使用 model_dump() 将 Pydantic 模型转换为字典以便查看内容
            span_data = span.model_dump()
            logger.info(f"Span {i} Name: {span.name}")
            # 打印核心内容：如输入输出、状态等
            logger.info(f"Span {i} Data: {json.dumps(span_data, indent=2, ensure_ascii=False)}")
        except Exception as e:
            logger.warning(f"Could not dump span {i}: {e}")
    logger.info("=== [DEBUG] End of Spans ===")
    # ========================================
...
async def dry_run_claude_code(
    *,
    ...
    try:
        ...
        swebench_full_dataset  = load_swebench_dataset_local(
            name="princeton-nlp/SWE-bench",
            split="test",
            local_root="/gfs/platform/public/infra/wyb/SWE-bench/data",
        )
